...
public class AutorizacionMiddleware implements Handler {
    ...
    private List<Permiso> permisosPlataforma = new ArrayList<>();
    private List<Permiso> permisosComunidad = new ArrayList<>();
    ...
    public void handle(Context context) throws Exception {
        ...
        if(this.chequearComunidad) {
            ...
            try { //por si falla el parseo a entero
                ...
                if(!this.permisosComunidad.stream().allMatch(permiso -> membresia.getRoles().stream().map(Rol::getPermisos).flatMap(List::stream).anyMatch(p -> p.getId() == permiso.getId()))){
                    //El usuario no tiene todos los permisos necesarios
                    throw ex;
                }
        ...
        if(!this.permisosPlataforma.stream().allMatch(permiso -> usuarioLogueado.getRoles().stream().map(Rol::getPermisos).flatMap(List::stream).anyMatch(p -> p.getId() == permiso.getId()))){
            //El usuario no tiene todos los permisos necesarios
            throw ex;
        }
    }
